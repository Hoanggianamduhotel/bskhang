...
const ReceptionistView: React.FC = () => {
  ...
  const vietHoaChuCaiDau = (str: string) => {
    return str
      .toLowerCase()
      .split(' ')
  ...
  const hienThiTuoiTheoThang = (thangTuoi: number) => {
    // Nếu tháng tuổi > 36 tháng, đổi sang hiển thị tuổi
    if (thangTuoi > 36) {
  ...
  const handleSubmit = async () => {
    // Viết hoa chữ cái đầu tên trước khi xử lý
    const hoTenFormatted = vietHoaChuCaiDau(benhNhan.ho_ten.trim());
